...
This project ties together Elm, Google Firebase, and Microsoft Azure AD OAuth2/OpenID Connect. It's based on [jluckyiv/firebase-azure-ad-auth](https://github.com/jluckyiv/firebase-azure-ad-auth), which I adapted from the Node 8 branch of [firebase/functions-samples/instagram-auth](https://github.com/firebase/functions-samples/tree/Node-8/instagram-auth). As the Firebase samples explain, [you must use the Blaze pay-as-you-go plan](https://github.com/firebase/functions-samples/tree/Node-8/instagram-auth#setup-the-sample) for the project to work. The Blaze plan includes a lot of free use, but check [the pricing page](https://firebase.google.com/pricing/) for your needs.

This sample uses Azure AD's OpenIDConnect authentication to sign in to Azure. After authenticating, it uses OAuth 2.0 authorization to get Azure AD user information. After logging into Azure, the app creates a Firebase Custom Token using the `oid` from the Azure AD `id_token`, then creates a Firebase user with the Azure token.

This is a work in progress. The current commit is functional, but the code is pretty awful.

## Prerequisites

- [Node 8.15.0 LTS](https://nodejs.org/en/blog/release/v8.15.0/) because that's the Node runtime that Firebase Cloud Functions [supports](https://cloud.google.com/functions/docs/concepts/nodejs-8-runtime) as of this writing. I use [asdf](https://github.com/asdf-vm) to manage my versions, but any Node version manager should work.
- [Firebase CLI](https://github.com/firebase/firebase-tools)
- [Yarn](https://yarnpkg.com/en/) package manager (or whatever you like)
...
```shell
git clone https://github.com/jluckyiv/elm-firebase-azure
yarn
```

### Create and setup the Firebase project

Create the Firebase project first, because the Azure AD settings will require a redirect URI, which you'll get from your Firebase settings. At a minimum, set up `hosting` and `functions`.

[These instructions](https://firebase.google.com/docs/web/setup?authuser=0) explain how to create a Firebase app. Create the app, then _read_ the instructions for [adding Firebase to your web app](https://firebase.google.com/docs/web/setup?authuser=0#add_firebase_to_your_app), but _don't_ add the JSON to your project. Instead, add the configuration values to a `.env` file, as explained below.

1. Create a Firebase project using the [Firebase Developer Console](https://console.firebase.google.com) or the [Firebase CLI](https://firebase.google.com/docs/cli/#initialize_a_firebase_project). You must enable _hosting_ and _functions_.
1. Switch to the **Blaze** plan to [enable billing](https://console.firebase.google.com/project/_/overview?purchaseBillingPlan=metered) on your Firebase project. You must upgrade so Firebase Cloud Functions can execute external HTTP requests.
...
1. Make sure `service-account.json` is in `.gitignore` (which is the project default).

### Create and set up your Azure AD app

1. Register an Azure AD app on [Azure Portal App registrations](https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/RegisteredAppsPreview). Use an existing app or use **New registration** to create a new app.
1. Specify the app's callback URL in the list of **Redirect URIs** for your Azure AD app. Whitelist `https://localhost:3000/auth` for local development and `https://<application-id>.firebaseapp.com/auth` for production.
1. Find the **Application (client) ID** in the Overview of the App Registration page. Save it with the Firebase CLI. (See instructions in next section.)
1. Save **Directory (tenant) ID** from the Overview page with the Firebase CLI.
1. Under **Certificates & secrets**, generate a new client secret. Copy it immediately, because it's displayed only once. Save this value with the Firebase CLI.

### Save your Azure AD credentials as Firebase Cloud Functions environment variables

Avoid putting your Azure AD credentials in a file that can be mistakenly pushed to a public repo. Instead configure the environment [using the Firebase CLI](https://firebase.google.com/docs/functions/config-env)

```shell
firebase functions:config:set azure.client_id="your application/client id"
firebase functions:config:set azure.client_secret="your client secret"
firebase functions:config:set azure.tenant_id="your directory/tenant id"
```

### Save your redirect uri to the Functions environment

For development, you'll want to point Functions to your local server. For production, use your production server. Use the following commands. You must `deploy` the functions after modifying.

```shell
# dev
firebase functions:config:set azure.redirect_uri="http://localhost:3000/auth"
firebase deploy --only functions

# production
firebase functions:config:set azure.redirect_uri="https://<application-id>.firebaseapp.com/auth"
firebase deploy --only functions
```

### Use `dotenv` to configure Firebase for the client app

This project uses `dotenv` to store Firebase initialization values for the client. The project's `.gitignore` file automatically excludes the `.env` file from version control. Copy the project's `sample.env` template to `.env` and fill in the values.

Follow [these instructions](https://firebase.google.com/docs/web/setup?authuser=0#add_firebase_to_your_app) to get Firebase initialization variables. You'll need to take the JSON values you get and copy them to the `.env` file. Follow [these create-elm-app instructions](https://github.com/halfzebra/create-elm-app/blob/master/template/README.md#using-custom-environment-variables) to add these values to your `.env` file.

For example, this JSON value

```json
{
  "authDomain": "some-project-id.firebaseapp.com"
}
```

becomes this `.env` value.

```shell
ELM_APP_FIREBASE_AUTH_DOMAIN="some-project-id.firebasepp.com"
```

## Run the dev server

1. Run `yarn start` to run your dev server. Make sure your function `redirect_uri` points to `localhost` and not your Firebase app.
1. Login and make sure everything's working.
1. Click on the **Sign in with Azure AD** button and the page should redirect to the Microsoft auth page. Sign in and/or authorize the authentication request.
...
## Deploy your hosting project

1. Run `yarn deploy` to deploy the client. That command will build the app into `./build` and deploy to Firebase. Make sure your function points to your Firebase app.
1. Open the sample's website by using `firebase open hosting:site` or directly accessing `https://<project-id>.firebaseapp.com/`.
...
After the user has granted approval the user is redirected back to the `/auth` page along with an OAuth 2.0 Auth Code as a URL parameter. This Auth code is then sent to the `token` Function using a JSONP Request. The `token` function then:
...
- Returns the Custom Auth Token, oid, email, user display name and Azure AD access token to the `/auth` page.

The `/auth` page receives the Custom Auth Token and other data back from the AJAX request to the `token` Function and uses it to update the user's profile, saves the access token to the database, authenticate the user in Firebase and then redirect to the index page.
